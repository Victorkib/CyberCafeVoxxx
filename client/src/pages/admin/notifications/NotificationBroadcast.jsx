...
const NotificationBroadcast = () => {
  ...
  const fetchUsers = async () => {
    ...
    try {
      const response = await apiRequest.get(`${import.meta.env.VITE_API_URL}/api/users?limit=100`)
  ...
  return (
    <div className="bg-white p-4 rounded-lg shadow">
      ...
      <form onSubmit={handleSubmit}>
        ...
        {!broadcastToAll && (
          <div className="mb-4">
            <label className="block text-sm font-medium mb-1">Select Users ({selectedUsers.length} selected)</label>
            {loadingUsers ? (
              ...
              <div className="border rounded max-h-60 overflow-y-auto p-2">
                {users.length > 0 ? (
                  users.map((user) => (
                    <label key={user._id} className="flex items-center p-1 hover:bg-gray-50">
                      <input
                        type="checkbox"

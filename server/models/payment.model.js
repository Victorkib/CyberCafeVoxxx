...
import { PAYMENT_STATUS, PAYMENT_METHODS, PAYMENT_VALIDATION } from '../constants/payment.js';
...
const paymentSchema = new mongoose.Schema({
  ...
  amount: {
    type: Number,
    required: true,
    min: [PAYMENT_VALIDATION.MIN_AMOUNT, `Amount must be at least ${PAYMENT_VALIDATION.MIN_AMOUNT}`],
    max: [PAYMENT_VALIDATION.MAX_AMOUNT, `Amount cannot exceed ${PAYMENT_VALIDATION.MAX_AMOUNT}`]
  },
  method: {
    type: String,
    required: true,
  ...
  expiresAt: {
    type: Date,
    default: () => new Date(Date.now() + PAYMENT_VALIDATION.TRANSACTION_TIMEOUT_MINUTES * 60000)
  }
}, {
  timestamps: true
});
...
paymentSchema.methods.processRefund = async function(reason, refundedBy) {
